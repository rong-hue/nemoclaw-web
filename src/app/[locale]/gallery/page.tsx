...
export default function Gallery() {
  ...
  const works = (t.raw('works') as Array<{ title: string; author: string; tag: string }>).map(
    (w, i) => ({ ...w, id: i + 1, color: WORK_COLORS[i] })
  );
      ...
      <div className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {works.map((work) => (
            <div
              key={work.id}
              className="group rounded-3xl overflow-hidden bg-slate-900 border border-slate-800 hover:border-slate-600 transition-all hover:-translate-y-1 cursor-pointer"
            >
